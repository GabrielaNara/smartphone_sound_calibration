...
device_model_reference = "ref iphone15" #ADD THE model of reference device, in this example: "Class1 SLM1" and "ref iphone15"
...
calibration_table_mobile =  pd.read_excel(path_ + "/dataset/input.xlsx",sheet_name="mobile_sensors") # PRE-CALIBRATION VALUES AND DEVICE MODEL  
...
#location = 'entrance' 
for location in calibration_table['location'].dropna().unique():
    try: 
        df = pd.read_excel(path_ +f"/outputs/frequency_domain_by_sensor/frequency_{sensor_number}_{location}.xlsx") 
        
        # adding device_model information
        df['device_model'] = df['device'].map(
            calibration_table_mobile.set_index('device')['device_model'])
        ...
        os.makedirs(path_+'/outputs/reports', exist_ok=True)
        
        def generate_report(df, sensor_number, path_):
        
            row = df[df['device'] == sensor_number].iloc[0]
        
            figA = os.path.join(path_, "outputs", "frequency_domain_by_sensor",f"frequency_{sensor_number}_{location}.png")
            ...
            doc = SimpleDocTemplate(
                os.path.join(path_, f"outputs/reports/report_{sensor_number}_{location}.pdf"),
                pagesize=A4,
                leftMargin=60,
                rightMargin=60,
                topMargin=40,
